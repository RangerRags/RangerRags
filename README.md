 Welcome to my GitHub! 🗡️
<img align="right" width="250" src="./indie fantasy GIF by undungeon.gif">

I'm a first-year CS student at UCF diving deep into game development.
Currently building my skills from the ground up — one project at a time.

## 🎮 About Me

- 🎓 Freshman @ University of Central Florida — Computer Science    
- 🕹️ Aspiring Game Developer
- 💻 Languages: C · Java · Python
- 🔧 Exploring: Raylib for 2D game development
- 🌱 Currently learning: data structures, OOP, and game loops

## 🛠️ Projects

- ⚔️ **TextBasedAdventureGame** — A text-based adventure written in C
- 🎮 **RaylibPractice** — Experimenting with the Raylib game library

## 🚀 Goals

- Build and ship my first complete game
- Learn a game engine (Unity or Godot)
- Contribute to open source game projects

📍 Orlando, FL
